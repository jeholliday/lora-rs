...
//! It demonstrates receiving LoRa packets with CRC status, showing RSSI and SNR
//! for both valid and CRC-failed packets.
#![no_std]
#![no_main]
...
async fn main(_spawner: Spawner) {
    ...
    loop {
        ...
        match lora.rx_with_crc_status(&rx_pkt_params, &mut receiving_buffer).await {
            Ok((received_len, status, true)) => {
                info!(
                    "rx ok: len={}, rssi={}, snr={}",
                    received_len, status.rssi, status.snr
                );
            }
            Ok((received_len, status, false)) => {
                info!(
                    "rx crc fail: len={}, rssi={}, snr={}",
                    received_len, status.rssi, status.snr
                );
            }
